...
that the kernel module is loaded is to load it on boot.  For AMD CPUs
the module is called `kvm-amd`.  For Intel CPUs the module is called
`kvm-intel`.  Please review `/etc/rc.modules` for how to load these.

Make sure your user is in the `kvm` group.  Just run the following
